...
let s:VimInputMethod  = [" ====  introduction     ==== {{{"]
" =================================================
"    File: vim_input_method.vim
"  Author: 
" License: 
"  Readme: VimInputMethod is a Vim plugin as an Input Method for i_CTRL-^ in Vim
"    (1) do Chinese input without mode change: Midas touch
"  PnP: Plug and Play
"    (1) 
"  Usage: VimInputMethod takes advantage of the definition from Vim
"    (1) :help i_CTRL-^  Toggle the use of language      ...
"    (2) :help i_CTRL-_  Switch between languages        ...

" ============================================= }}}
let s:VimInputMethod += [" ====  initialization   ==== {{{"]
" =================================================

function! s:vim_input_method_bare_bones_vimrc()
...
if exists("g:Vim_input_method_profile") || &iminsert == 1 || v:version < 700
    finish
elseif &compatible
    call s:vim_input_method_bare_bones_vimrc()
endif

scriptencoding utf-8
let g:Vim_input_method_profile = reltime()
let s:plugin = expand("<sfile>:p:h")

function! s:vim_input_method_initialize_global()
    highlight  default lCursorIM guifg=NONE guibg=green gui=NONE
    highlight! link lCursor lCursorIM
    let s:space = '　'
    let s:colon = '：'
    let g:Vim_input_method = "VimInputMethod　中文輸入法"
    ...
    let s:rc = {}
    let s:rc["g:Vim_input_method_mode"] = 'dynamic'
    let s:rc["g:Vim_input_method_shuangpin"] = 0
    let s:rc["g:Vim_input_method_toggle"] = 0
    let s:rc["g:Vim_input_method_plugin"] = s:plugin
    let s:rc["g:Vim_input_method_punctuation"] = 2
    call s:vim_input_method_set_global_default()
    let s:plugin = isdirectory(g:Vim_input_method_plugin) ? g:Vim_input_method_plugin : s:plugin
    let s:plugin = s:plugin[-1:] != "/" ? s:plugin."/" : s:plugin
    let s:dynamic    = {'dynamic':1,'static':0}
    let s:static     = {'dynamic':0,'static':1}
endfunction

function! s:vim_input_method_dictionary_keycodes()
    let s:keycodes = {}
    ...
    let s:all_vim_input_method_input_methods = keys(s:keycodes) + split(ime)
endfunction

function! s:vim_input_method_set_frontend()
    let quote = 'erbi wu nature yong boshiamy'
    let s:valid_keyboard = "[0-9a-z']"
    if !empty(s:ui.root) && empty(g:Vim_input_method_shuangpin)
        let s:valid_keyboard = s:backend[s:ui.root][s:ui.im].keycode
    elseif g:Vim_input_method_shuangpin == 'ms' || g:Vim_input_method_shuangpin == 'purple'
        let s:valid_keyboard = "[0-9a-z';]"
    endif
    let i = 0
    let keycode_string = ""
    ...
    if g:Vim_input_method_punctuation > 0 && s:toggle_punctuation > 0
        let tail = s:chinese('fullwidth')
    endif
    let g:Vim_input_method = "VimInputMethod".s:space.logo.' '.s:vim_input_method_im_chinese().' '.tail
endfunction

function! s:vim_input_method_set_global_default()
    let s:vim_input_methodrc = []
    let s:vim_input_methoddefaults = []
    for variable in keys(s:rc)
        if exists(variable)
            let value = string(eval(variable))
            let vim_input_methodrc = ':let ' . variable .' = '. value .' '
            call add(s:vim_input_methodrc, '    ' . vim_input_methodrc)
        else
            let value = string(s:rc[variable])
            let vim_input_methodrc = ':let ' . variable .' = '. value .' '
            call add(s:vim_input_methoddefaults, '  " ' . vim_input_methodrc)
        endif
        exe 'let '. variable .'='. value
    endfor
endfunction

function! s:vim_input_method_cache()
    let results = []
...
" ============================================= }}}
let s:VimInputMethod += [" ====  user interface   ==== {{{"]
" =================================================

function! s:vim_input_method_dictionary_punctuations()
    let s:antonym = " 〖〗 （） 《》 【】 ‘’ “”"
    let one =       " { }  ( )  < >  [  ] "
    let two = join(split(join(split(s:antonym)[:3],''),'\zs'))
    let antonyms = s:vim_input_method_key_value_hash(one, two)
    let one = " ,  .  +  -  ~  ^    _    "
    let two = " ， 。 ＋ － ～ …… —— "
    let mini_punctuations = s:vim_input_method_key_value_hash(one, two)
    let one = " @  :  #  &  %  $  !  =  ;  ?  * "
    let two = " 　 ： ＃ ＆ ％ ￥ ！ ＝ ； ？ ﹡"
    let most_punctuations = s:vim_input_method_key_value_hash(one, two)
    ...
    let s:punctuations = {}
    if g:Vim_input_method_punctuation > 0
        call extend(s:punctuations, mini_punctuations)
    endif
    if g:Vim_input_method_punctuation > 1
        call extend(s:punctuations, most_punctuations)
    endif
endfunction

function! g:Vim_input_method_bracket(offset)
    let cursor = ""
...
function! s:vim_input_method_get_label(label)
    let labeling = a:label == 10 ? "0" : a:label
    return labeling
endfunction

function! s:vim_input_method_set_pumheight()
...
function! s:vim_input_method_im_chinese()
    if empty(s:ui.im)
    ...
    if !empty(g:Vim_input_method_shuangpin)
        let title = s:chinese(s:space, g:Vim_input_method_shuangpin, 'shuangpin')
    endif
    if g:Vim_input_method_shuangpin =~ 'abc'
        let title = substitute(title,s:chinese('pin'),s:chinese('hit'),'')
    endif
    return title
endfunction

function! g:Vim_input_method_esc()
    let key = nr2char(27)  "  <Esc> is <Esc>
    sil!exe 'sil!return "' . key . '"'
endfunction

" ============================================= }}}
let s:VimInputMethod += [" ====  lmap imap nmap   ==== {{{"]
" =================================================

function! g:Vim_input_method_cycle_vim_input_method()
    if s:mode.static || s:mode.dynamic
        let s:toggle_punctuation = (s:toggle_punctuation + 1) % 2
    endif
    sil!call s:vim_input_method_set_frontend()
    sil!call s:vim_input_method_set_keyboard_maps()
    return ""
endfunction

function! g:Vim_input_method_label(key)
    let key = a:key
    if pumvisible()
        ...
        let omni = '\<C-R>=g:Vim_input_method()\<CR>'
        if len(yes)
            sil!call s:vim_input_method_reset_after_insert()
        endif
        let key = yes . omni
    endif
    sil!exe 'sil!return "' . key . '"'
...
function! g:Vim_input_method_page(key)
    let key = a:key
    if pumvisible()
        let page = '\<C-E>\<C-R>=g:Vim_input_method()\<CR>'
        if key =~ '[][]'
            let left  = key == "]" ? "\<Left>"  : ""
            let right = key == "]" ? "\<Right>" : ""
            let _ = key == "]" ? 0 : -1
            let backspace = '\<C-R>=g:Vim_input_method_bracket('._.')\<CR>'
...
function! s:vim_input_method_punctuation_maps()
...
function! g:Punctuation(key)
    ...
    elseif s:gi_dynamic
        ...
        call g:Vim_input_method_space()
    endif
    sil!exe 'sil!return "' . key . '"'
endfunction

function! g:Vim_input_method_pagedown()
    let key = ' '
    if pumvisible()
        let s:pageup_pagedown = &pumheight ? 1 : 0
        let key = &pumheight ? g:Vim_input_method() : '\<PageDown>'
    endif
    sil!exe 'sil!return "' . key . '"'
endfunction

function! g:Vim_input_method_space()
    ...
    let key = " "
    if pumvisible()
        let key = '\<C-R>=g:Vim_input_method()\<CR>'
    ...
    elseif s:mode.static
        let key = s:vim_input_method_left() ? g:Vim_input_method() : key
    ...
    call s:vim_input_method_reset_after_insert()
    sil!exe 'sil!return "' . key . '"'
endfunction

function! g:Vim_input_method_enter()
    let s:omni = 0
    let key = ""
    if pumvisible()
        let key = "\<C-E>"
        let s:smart_enter = 1
    elseif s:vim_input_method_left()
        let s:smart_enter = 1
...
" ============================================= }}}
let s:VimInputMethod += [" ====  mode: chinese    ==== {{{"]
" =================================================

function! g:Vim_input_method_chinese()
    let s:mode = g:Vim_input_method_mode =~ 'static' ? s:static : s:dynamic
    let s:switch = empty(s:ui.frontends) ? -1 : s:switch ? 0 : 1
    return s:switch<0 ? "" : s:switch ? s:vim_input_method_start() : s:vim_input_method_stop()
endfunction

function! s:vim_input_method_set_keyboard_maps()
    ...
    if both_dynamic
        for char in s:valid_keys
            ...
            \ '<C-R>=g:Wubi()<CR>' . char . '<C-R>=g:Vim_input_method()<CR>'
        endfor
    elseif s:mode.static
        for char in s:valid_keys
    ...
    elseif both_dynamic || s:mode.static
        sil!call s:vim_input_method_punctuation_maps()
    endif
    for _ in common_punctuations
        if _ !~ s:valid_keyboard
            sil!exe 'lnoremap<buffer><expr> '._.' g:Vim_input_method_page("'._.'")'
        endif
    endfor
    for _ in common_labels
        sil!exe 'lnoremap<buffer><expr> '._.' g:Vim_input_method_label("'._.'")'
    endfor
endfunction

function! s:vim_input_method_set_im_toggle_list()
    let toggle_list = []
    if g:Vim_input_method_toggle < 0
        let toggle_list = [get(s:ui.frontends,0)]
    elseif empty(g:Vim_input_method_toggle)
        let toggle_list = s:ui.frontends
    else
        for toggle in split(g:Vim_input_method_toggle, ",")
...
function! s:vim_input_method_get_seamless(cursor_positions)
...
" ============================================= }}}
let s:VimInputMethod += [" ====  input: unicode   ==== {{{"]
" =================================================

function! s:vim_input_method_i18n(line)
    let line = a:line
    if s:localization == 1
...
function! s:vim_input_method_left()
    let key = 0
...
function! s:vim_input_method_key_value_hash(single, double)
    let hash = {}
...
function! s:vim_input_method_filereadable(filename)
...
function! s:vim_input_method_readfile(datafile)
    let lines = []
    if filereadable(a:datafile)
        if s:localization
            for line in readfile(a:datafile)
                call add(lines, s:vim_input_method_i18n(line))
            endfor
        else
...
" ============================================= }}}
let s:VimInputMethod += [" ====  input: pinyin    ==== {{{"]
" =================================================

function! s:vim_input_method_get_all_valid_pinyin_list()
...
function! s:vim_input_method_quanpin_transform(pinyin)
    if empty(s:quanpin_table)
        for key in s:vim_input_method_get_all_valid_pinyin_list()
            if key[0] == "'"
...
function! s:vim_input_method_more_pinyin_datafile(keyboard, sentence)
    let results = []
    let backend = s:backend[s:ui.root][s:ui.im]
    for candidate in s:vim_input_method_more_pinyin_candidates(a:keyboard)
        ...
        call extend(results, s:vim_input_method_make_pairs(oneline))
    endfor
    return results
endfunction

function! s:vim_input_method_get_pinyin(keyboard)
    let keyboard = s:vim_input_method_quanpin_transform(a:keyboard)
...
function! s:vim_input_method_more_pinyin_candidates(keyboard)
    " make standard menu layout:  mamahuhu => mamahu, mama
    if s:ui.im !~ 'pinyin' || !empty(g:Vim_input_method_shuangpin)
        return []
    endif
    let candidates = []
    let keyboards = s:vim_input_method_get_pinyin(a:keyboard)
    if len(keyboards)
...
" ============================================= }}}
let s:VimInputMethod += [" ====  input: shuangpin ==== {{{"]
" =================================================

function! s:vim_input_method_shuangpin_generic()
    let shengmu_list = {}
...
function! s:vim_input_method_shuangpin_rules(shuangpin, rules)
    let rules = a:rules
    ...
    call extend(rules[1], s:vim_input_method_key_value_hash(key, v))
    return rules
endfunction

function! s:vim_input_method_create_shuangpin_table(rules)
    let pinyin_list = s:vim_input_method_get_all_valid_pinyin_list()
    let sptable = {}
    ...
    if match(split("abc purple nature flypy"), g:Vim_input_method_shuangpin) > -1
        let jxqy = {"jv":"ju", "qv":"qu", "xv":"xu", "yv":"yu"}
        call extend(sptable, jxqy)
    elseif g:Vim_input_method_shuangpin == 'ms'
        let jxqy = {"jv":"jue", "qv":"que", "xv":"xue", "yv":"yue"}
        call extend(sptable, jxqy)
    endif
    if g:Vim_input_method_shuangpin == 'flypy'
        let key   = 'ou eg  er an ao ai aa en oo os  ah  ee ei'
        let value = 'ou eng er an ao ai a  en o  ong ang e  ei'
        call extend(sptable, s:vim_input_method_key_value_hash(key, value))
    endif
    if g:Vim_input_method_shuangpin == 'nature'
...
function! s:vim_input_method_shuangpin_transform(keyboard)
...
" ============================================= }}}
let s:VimInputMethod += [" ====  backend: file    ==== {{{"]
" =================================================

function! s:vim_input_method_set_datafile(im, datafile)
    let im = a:im
...
function! s:vim_input_method_sentence_datafile(keyboard)
    let backend = s:backend[s:ui.root][s:ui.im]
    ...
    let candidates = s:vim_input_method_more_pinyin_datafile(a:keyboard,1)
...
function! s:vim_input_method_get_from_datafile(keyboard)
    let fuzzy = s:ui.im =~ 'pinyin' ? ' ' : ""
    let pattern = '^\V' . a:keyboard . fuzzy
    let backend = s:backend[s:ui.root][s:ui.im]
    let cursor = match(backend.lines, pattern)
    ...
    if s:ui.im =~ 'pinyin'
        let extras = s:vim_input_method_more_pinyin_datafile(a:keyboard,0)
        let results = s:vim_input_method_make_pairs(oneline) + extras
    else
        let results = []
        ...
        for i in range(10)
            let cursor += i
            let oneline = get(backend.lines, cursor)
            let results += s:vim_input_method_make_pairs(oneline)
        endfor
    endif
    return results
endfunction

function! s:vim_input_method_make_pairs(oneline)
...
" ============================================= }}}
let s:VimInputMethod += [" ====  backend: dir     ==== {{{"]
" =================================================

function! s:vim_input_method_set_directory(dir)
    let im = "pinyin"
...
function! s:vim_input_method_sentence_directory(keyboard, directory)
...
function! s:vim_input_method_set_backend_embedded()
    let dir = s:plugin . "pinyin"
    if isdirectory(dir)
        if filereadable(dir . "/pinyin")
            return s:vim_input_method_set_directory(dir . "/")
        endif
    endif
    for im in s:all_vim_input_method_input_methods
        let datafile = s:vim_input_method_filereadable("vim_input_method." . im . ".txt")
        if empty(datafile)
            let filename = "vim_input_method." . im . "." . &encoding . ".txt"
            let datafile = s:vim_input_method_filereadable(filename)
        endif
        if !empty(datafile)
            call s:vim_input_method_set_datafile(im, datafile)
        endif
    endfor
endfunction

function! s:vim_input_method_sort_on_length(i1, i2)
    return len(a:i2) - len(a:i1)
endfunc

" ============================================= }}}
let s:VimInputMethod += [" ====  core workflow    ==== {{{"]
" =================================================

function! s:vim_input_method_start()
    sil!call s:vim_input_method_save_vimrc()
    sil!call s:vim_input_method_set_vimrc()
    sil!call s:vim_input_method_set_frontend()
    sil!call s:vim_input_method_set_keyboard_maps()
    lnoremap <silent><buffer> <expr> <Esc>   g:Vim_input_method_esc()
    lnoremap <silent><buffer> <expr> <C-L>   g:Vim_input_method_cycle_vim_input_method()
    if s:ui.im =~ 'array'
        lnoremap <silent><buffer> <expr> <CR>    g:Vim_input_method_space()
        lnoremap <silent><buffer> <expr> <Space> g:Vim_input_method_pagedown()
    else
        lnoremap <silent><buffer> <expr> <CR>    g:Vim_input_method_enter()
        lnoremap <silent><buffer> <expr> <Space> g:Vim_input_method_space()
    endif
    let key = ''
    if empty(s:ctrl6)
        let s:ctrl6 = 32911
...
function! s:vim_input_method_stop()
    if has("gui_running")
        lmapclear
    endif
    " i_CTRL-^
    ...
    sil!call s:vim_input_method_restore_vimrc()
    sil!call s:vim_input_method_super_reset()
    sil!exe 'sil!return "' . key . '"'
endfunction

function! s:vim_input_method_save_vimrc()
    let s:omnifunc    = &omnifunc
    let s:complete    = &complete
    let s:completeopt = &completeopt
endfunction

function! s:vim_input_method_set_vimrc()
    set title noshowmatch shellslash
    set completeopt=menuone
    set complete=.
    set nolazyredraw
    set omnifunc=VimInputMethod
endfunction

function! s:vim_input_method_restore_vimrc()
...
function! s:vim_input_method_super_reset()
    sil!call s:vim_input_method_reset_before_anything()
    sil!call s:vim_input_method_reset_before_omni()
    sil!call s:vim_input_method_reset_after_insert()
endfunction

function! s:vim_input_method_reset_before_anything()
...
function! s:vim_input_method_reset_before_omni()
    let s:show_extra_menu = 0
endfunction

function! s:vim_input_method_reset_after_insert()
    let s:match_list = []
...
" ============================================= }}}
let s:VimInputMethod += [" ====  core engine      ==== {{{"]
" =================================================

function! VimInputMethod(start, keyboard)
...
if a:start
    ...
    let seamless_column = s:vim_input_method_get_seamless(cursor_positions)
    ...
    let results = s:vim_input_method_cache()
    if empty(results)
        sil!call s:vim_input_method_reset_before_omni()
    else
        return s:vim_input_method_popupmenu_list(results)
    endif
    ...
    if !empty(g:Vim_input_method_shuangpin)
        if empty(s:shuangpin_table)
            let rules = s:vim_input_method_shuangpin_generic()
            let rules = s:vim_input_method_shuangpin_rules(g:Vim_input_method_shuangpin, rules)
            let s:shuangpin_table = s:vim_input_method_create_shuangpin_table(rules)
        endif
        if empty(s:has_shuangpin_transform)
            let keyboard = s:vim_input_method_shuangpin_transform(keyboard)
            let s:keyboard = keyboard
        endif
    endif
    if empty(results)
        ...
        let results = s:vim_input_method_embedded_backend_engine(keyboard)
    endif
    if empty(results)
        let s:pattern_not_found = 1
    endif
...
function! s:vim_input_method_popupmenu_list(lines)
    ...
    for chinese in s:match_list
        let complete_items = {}
        let titleline = s:vim_input_method_get_label(label)
        let menu = ""
    ...
    call s:vim_input_method_set_pumheight()
    return s:popup_list
endfunction

function! s:vim_input_method_embedded_backend_engine(keyboard)
    let keyboard = a:keyboard
    ...
    if backend.name =~ "quote" && keyboard !~ "[']"
        let keyboard = s:vim_input_method_quanpin_transform(keyboard)
    endif
    if s:ui.root =~# "directory"
        let head = s:vim_input_method_sentence_directory(keyboard, backend.name)
        let results = s:vim_input_method_readfile(backend.name . head)
        if keyboard ==# head && len(results) && len(results) < 20
            let extras = []
            for candidate in s:vim_input_method_more_pinyin_candidates(keyboard)
                let lines = s:vim_input_method_readfile(backend.name . candidate)
    ...
    elseif s:ui.root =~# "datafile"
        if empty(backend.lines)
            let backend.lines = s:vim_input_method_readfile(backend.name)
        endif
        let head = s:vim_input_method_sentence_datafile(keyboard)
        let results = s:vim_input_method_get_from_datafile(head)
    endif
    if s:keyboard !~ '\S\s\S'
        if empty(head)
            let s:keyboard = keyboard
...
function! g:Vim_input_method()
    let s:omni = s:omni < 0 ? -1 : 0
    let s:keyboard = empty(s:pageup_pagedown) ? "" : s:keyboard
    let key = s:vim_input_method_left() ? '\<C-X>\<C-O>\<C-R>=g:Omni()\<CR>' : ""
    sil!exe 'sil!return "' . key . '"'
endfunction
...
" ============================================= }}}
let s:VimInputMethod += [" ====  core driver      ==== {{{"]
" =================================================

function! s:vim_input_method_plug_and_play()
    nnoremap <silent> <C-_> a<C-R>=g:Vim_input_method_chinese()<CR>
    inoremap <unique> <C-_>  <C-R>=g:Vim_input_method_chinese()<CR>
endfunction

sil!call s:vim_input_method_initialize_global()
sil!call s:vim_input_method_dictionary_punctuations()
sil!call s:vim_input_method_dictionary_keycodes()
sil!call s:vim_input_method_super_reset()
sil!call s:vim_input_method_set_backend_embedded()
sil!call s:vim_input_method_set_im_toggle_list()
sil!call s:vim_input_method_plug_and_play()
:let g:Vim_input_method_profile = reltime(g:Vim_input_method_profile)
" ============================================= }}}
